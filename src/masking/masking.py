...
from src.masking.utils import get_mask_from_weight_score_vec, sample_mask_from_weight_score_vec
...
def wasserstein_mask(model, batchnorm_layers, n_weights_subnet, train_loader, device, loss="cross_entropy", n_snapshots=256, swag_lr=1e-2, swag_c_epochs=1, swag_c_batches=None, parallel=False, last_layer=False):
    ...
    weight_score_vec = compute_wasserstein_swag_weight_score_vec(model, train_loader, batchnorm_layers, device=device, loss=loss, n_snapshots=n_snapshots, swa_lr=swag_lr, swa_c_epochs=swag_c_epochs, swa_c_batches=swag_c_batches, parallel=parallel)
    # return get_mask_from_weight_score_vec(model, weight_score_vec, n_weights_subnet, batchnorm_layers, largest=True, last_layer=last_layer)
    return get_mask_from_layer_aware_weight_score_vec(model, weight_score_vec, n_weights_subnet, batchnorm_layers, largest=True, last_layer=last_layer)
